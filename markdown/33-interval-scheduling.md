...
### Drawbacks
Redundant recomputation; does not exploit the structure the Interval Scheduling pattern is built to use.
...
3. Introduce the Interval Scheduling invariant/structure so each element/query costs far less.
...
"This is a Interval Scheduling problem. I'll sorting linearizes the geometry so a single left-to-right sweep resolves all overlaps. That brings the complexity down to O(n log n) time and O(n) space — here's the template."
...
### Visualization
```
input  ──▶ [ apply Interval Scheduling step-by-step ]
