...
### Drawbacks
Redundant recomputation; does not exploit the structure the Trie pattern is built to use.
...
3. Introduce the Trie invariant/structure so each element/query costs far less.
...
A representative **Trie** problem. The signal: prefix tree for fast word/prefix lookups and autocomplete.

### Thought Process
1. Confirm the pattern via its recognition signals (trie, prefix tree, autocomplete, word dictionary, insert search).
2. Reach for the Trie template below and map the problem's entities onto it.
