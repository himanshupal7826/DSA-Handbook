...
### Drawbacks
Redundant recomputation; does not exploit the structure the Median of Stream pattern is built to use.
...
3. Introduce the Median of Stream invariant/structure so each element/query costs far less.
...
Median of Stream  : maintain state, update in O(1)/O(log n) ──▶ fast
```

### Interview explanation
"This is a Median of Stream problem. I'll a heap gives O(1) access to the extreme element and O(log n) updates — perfect for top-k, merging, and running medians. That brings the complexity down to O(n log k) time and O(k) space — here's the template."
...
A representative **Median of Stream** problem. The signal: online median via balanced two-heap, o(log n) insert, o(1) query.

### Thought Process
1. Confirm the pattern via its recognition signals (median, stream, two heaps, online, running median).
2. Reach for the Median of Stream template below and map the problem's entities onto it.
...
### Visualization
```
input  ──▶ [ apply Median of Stream step-by-step ]
...
### Problem — Moving Average (LeetCode 346)
A representative **Median of Stream** problem. The signal: online median via balanced two-heap, o(log n) insert, o(1) query.
