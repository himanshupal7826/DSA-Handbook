...
A representative **Floyd Warshall** problem. The signal: all-pairs shortest paths via dp over intermediate vertices, o(v³).

### Thought Process
1. Confirm the pattern via its recognition signals (floyd warshall, all pairs, shortest path, dp, transitive closure).
2. Reach for the Floyd Warshall template below and map the problem's entities onto it.
