...
### Drawbacks
Redundant recomputation; does not exploit the structure the Bitmask DP pattern is built to use.
...
### Problem — Shortest Visiting (LeetCode 847)
A representative **Bitmask DP** problem. The signal: encode subsets as bitmasks to dp over 2^n states.

### Thought Process
1. Confirm the pattern via its recognition signals (bitmask dp, subset state, tsp, assignment, exponential).
2. Reach for the Bitmask DP template below and map the problem's entities onto it.
...
### Visualization
```
input  ──▶ [ apply Bitmask DP step-by-step ]
