...
### Drawbacks
Redundant recomputation; does not exploit the structure the Shortest Window pattern is built to use.
...
3. Introduce the Shortest Window invariant/structure so each element/query costs far less.
...
Shortest Window   : maintain state, update in O(1)/O(log n) ──▶ fast
```

### Interview explanation
"This is a Shortest Window problem. I'll a window with incrementally maintained aggregates means each element enters and leaves at most once — amortized O(n). That brings the complexity down to O(n) time and O(k) space — here's the template."
...
### Problem — Balanced String (LeetCode 1234)
A representative **Shortest Window** problem. The signal: minimize window length; shrink aggressively while validity holds.
