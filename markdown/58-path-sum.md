...
### Drawbacks
Redundant recomputation; does not exploit the structure the Path Sum pattern is built to use.
...
3. Introduce the Path Sum invariant/structure so each element/query costs far less.
...
"This is a Path Sum problem. I'll trees are recursive: solve children first, combine their results at the parent. BFS handles level-aggregates. That brings the complexity down to O(n) time and O(h) space — here's the template."
...
A representative **Path Sum** problem. The signal: dfs carrying running sum; optionally prefix-sum for any-path counts.

### Thought Process
1. Confirm the pattern via its recognition signals (path sum, root to leaf, dfs, target, prefix sum tree).
2. Reach for the Path Sum template below and map the problem's entities onto it.
...
### Visualization
```
input  ──▶ [ apply Path Sum step-by-step ]
