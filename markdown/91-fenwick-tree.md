...
### Drawbacks
Redundant recomputation; does not exploit the structure the Fenwick Tree pattern is built to use.
...
3. Introduce the Fenwick Tree invariant/structure so each element/query costs far less.
...
A representative **Fenwick Tree** problem. The signal: binary indexed tree: compact o(log n) prefix sums with updates.

### Thought Process
1. Confirm the pattern via its recognition signals (fenwick, binary indexed tree, bit, prefix sum, point update).
2. Reach for the Fenwick Tree template below and map the problem's entities onto it.
...
### Visualization
```
input  ──▶ [ apply Fenwick Tree step-by-step ]
