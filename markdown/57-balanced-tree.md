...
### Drawbacks
Redundant recomputation; does not exploit the structure the Balanced Tree pattern is built to use.
...
3. Introduce the Balanced Tree invariant/structure so each element/query costs far less.
...
"This is a Balanced Tree problem. I'll trees are recursive: solve children first, combine their results at the parent. BFS handles level-aggregates. That brings the complexity down to O(n) time and O(h) space — here's the template."
...
### Problem — Balanced Tree (LeetCode 110)
A representative **Balanced Tree** problem. The signal: return height and a balance flag together to check balance in o(n).

### Thought Process
1. Confirm the pattern via its recognition signals (balanced, height balanced, avl, recursion, subtree).
2. Reach for the Balanced Tree template below and map the problem's entities onto it.
...
### Visualization
```
input  ──▶ [ apply Balanced Tree step-by-step ]
...
### Problem — Balance BST (LeetCode 1382)
A representative **Balanced Tree** problem. The signal: return height and a balance flag together to check balance in o(n).
